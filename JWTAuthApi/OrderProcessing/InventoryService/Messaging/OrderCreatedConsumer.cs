...
public class OrderCreatedConsumer : BackgroundService
{
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var queue = _channel.QueueDeclare("inventory.orders.created", durable: true, exclusive: false, autoDelete: false);
        ...
        consumer.Received += async (_, ea) =>
        {
            try
            {
                ...
                await inventoryService.HandleOrderCreatedAsync(evt, stoppingToken);
            }
            catch (Exception ex)
            {
